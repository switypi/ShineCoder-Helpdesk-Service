    ...
    public class TicketsController : ControllerBase
    {
        ...
        public async Task<JObject> GetTicketsByIdAsync()
        {
            try
            {
                ...
                var data = await GetTicketDetails(searchModel);
        ...
        private async Task<List<TicketsModel>> GetTicketDetails(SearchModel searchModel)
        {
            try
            {
                ...
                else
                {
                    ...
                    {
                        if (searchModel.UserName.ToLower() != nameof(UserTypeEnum.ADMIN).ToLower())
                        {
                            var userObj = db.Users.FirstOrDefault(x => x.UserName == searchModel.UserName);
                            if (userObj != null)
                                tksQuery = tksQuery.Where(x => x.Tkt_RequestUserId == userObj.Id);
                            else
                                return result;
                        }
                    }
